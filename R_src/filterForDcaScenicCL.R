...
#Add Tf in DEG
importantTF <- read.table(opt$importantTF)$V1

tfToAdd <- importantTF[which(importantTF %in% genesLeftMinCells)  == F] 

t_data <- t(data)

print(head(genesLeftMinCells))

write.table(t_data[,c(genesLeftMinCells,tfToAdd)],file = paste(opt$outdir,"/expressionRawCountFilteredForScenic.tsv",sep=""),sep = '\t',quote = F)

allTF <- TF$external_gene_name[which(TF$external_gene_name %in% c(genesLeftMinCells,tfToAdd))]

#Write TF list for scenic
write.table(allTF,paste(opt$outdir,"/allTFforScenic.tsv",sep =""),
